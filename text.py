...
stop.update(['would', 'should', 'said', 'also', 'n\'t', 'wouldnt', 'get', 'told'])
sentence_mapping = {}
ctr = Counter()
...
def generate_bar_data(df, title):
    ...
    fig = px.bar(df.reset_index(drop=True), x='Word', y='Frequency', title=title)  
    fig.update_layout(
...
def generate_word_cloud(df):
    ...
    wc = WordCloud(random_state=2, width=800, height=400, colormap='Set3', background_color='#1e1e1e')
